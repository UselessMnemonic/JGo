...
## Mapping Go to Java
### Calling Conventions
In Java, primitives (like `int`, `double`, and the internal `ref`) are passed into functions by
their values alone; this makes them pass-by-value. Java objects (meaning the data comprising object
types) are passed by their references, so objects in Java are _always_ pass-by-reference.
...
#### Aliases
Aliases are, as their name suggests, aliases. They do not appear in the final Java source. All
occurrences of an aliased type are replaced.

### Defined Types
When a new type is defined from a basic type, the new type extends the basic type in Java.

### Nil and Default Values
Go assigns default values to variables lacking an assignment. For value types like integers, the
default value is `0`; for strings, the empty string; for arrays, an array of default-ly initialized
elements.
...
that `nil` between differing types is not comparable (i.e. `(*int)(nil) != (*bool)(nil)`).
...
```
All structs derive from the `Struct` class, which exposes some reflection capabilities specific to
structs. All structs acquire autogenerated implementations of `equals()`, `hashCode()`, and
`clone()`.
...
```
For Java, the following declarations are made from the above example:
```java
public class Base {
    Int8 b;
}
...
that return multiple values instead return an immutable tuple: any of `Couple`, `Triple`, and so on
up to `Sextuple` and the general-purpose `NTuple`. Here is an example:
```golang
...
interface. The method set of any type `T` consists of all methods declared with receiver type `T`.
The method set of type `*T` also contains the method set of type `T`.
...
When an empty interface variable is declared, its dynamic type and value are `nil`. This in turn
means the interface value is `nil`. A non-empty interface variable starts with its dynamic value as
`nil`. 
...
#### The Interface Class
To rectify this issue, first we introduce the `Interface` class, the parent of all Go interfaces.
Assuming `I` is a Go interface, instances of `I` can be set to values that implement `I` or
to `Pointer`s whose underlying values implement `I`.

Now the above example can be written as such:
```java
Concrete c = new Concrete();
EX i1 = Interface.type(EX.class, c.clone());
EX i2 = Interface.type(EX.class, new Pointer(c))
```
This example can even be extended to pointers _of_ interfaces:
```java
EX i3 = Interface.type(EX.class, new Pointer(i1));
```
...
Internally, `recover()` translates into a `try-catch` block that captures any exceptions.
...
## Anonymity
The last, most annoying topic to tackle may be the applications of anonymity in Go. Go allows for
anonymous interfaces, structs, and functions, in various contexts. In JGo, anonymous elements
correspond to generated types within the packages in which they are used.

Autogenerated elements still require identifiers in some contexts. For these identifiers, a hash
of the anonymous element is appended to any of `Struct_`, `Interface_`, or `Func_`.

### Anonymous Structs
Go allows for _anonymous structs_, structs whose types are in a sense named by their declarations.
Take this example:
```golang
s := struct {
    item string
...
```
The type of `s` resolves to `struct { item string }`. This is in contrast to :
```golang
...
```
Here, the type of `s` resolves to `StructType`. As a side-note, these types are _identical_ because
they aren't **both** _defined_ types. (Read the [properties of Go types](https://golang.org/ref/spec#Properties_of_types_and_values)
for more information).

The JGo solution is to generate anonymous classes in place of anonymous structs, as well as to
decorate function parameters and variables with annotations that list the required fields of those
structs.

### Anonymous Interfaces
Another quirk of Go is that interfaces can be anonymous. Assuming this definition for `Concrete`:
```go
type Concrete struct {}

func (c Concrete) Method() string {...}
```
The following are legitimate:
```go
func returnInterfaceType() interface {
	Method() string
} {
	return Concrete{}
}
...
var anon interface {
 ...
 ```
Legitimate, but ugly. Reluctantly, JGo supports this construct in a way similar to how anonymous
structs are supported. Anonymous interfaces are replaced with `Interface` and decorated with
annotations that list the required methods of the underlying value types.

### Anonymous Functions
The Java equivalent of the Go anonymous function is the lambda. It goes without saying, that lambdas
take the place of anonymous functions in this context.